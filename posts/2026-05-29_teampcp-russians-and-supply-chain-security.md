...
- Nation-state repositories tied to government agencies could be funded directly by the public. This might ensure the active involvement of citizens in the process, similar to how taxes fund municipal services. Thus, people will become more responsible and aware that software is an integral part of their lives, rather than just a free service that shouldn't be paid for. For example, public and independent organizations like Codeberg in Germany rely on donations or contributions on a voluntary basis, changing the perspective on software from consumeristic to collaborative while ensuring sustainability and maintenance of projects. It can combined with subscription models based on a [Perpetual Fallback License][perpetual_fallback_license] and the [Open-Core model][open_core_model], which do not restrict access to versions already owned at the time of purchase.
- In the context of critical IT infrastructure, there should be regional/national registry of critical software (e.g., cryptographic libraries, operating system distributions, compilers, interpreters, etc.) from various sources (external and internal) that provides continuous monitoring and certification/indexing of specific software versions. It means that only approved versions would be allowed for deployment across banking systems, quasi-public sector, and other critical industries. This approach could potentially prevent software supply chain attacks, because there are too many unverified sources and non-official repositories. Practically, such platform should be managed by a consortium of multiple cybersecurity organizations in Kazakhstan in order to avoid: monopolization and concentration of power within a single entity; operational bottlenecks due to heavy workloads; corruption and vendor bias; weak analysis in specialized expertise. Currently, the State Technical Service performs a similar role in Kazakhstan by conducting source code analysis and in-laboratory testing, but it is not sufficient due to lack of independent laboratories and diversity of industry actors. We should ensure transparency in any local software products/projects (composition, sources, codebases, processes, costs, schedule, efficiency) during maintenance or initial creation. Moreover, Figure 3 shows my concept in which AI agents can reduce the workload on cybersecurity specialists and cut redundant bureaucracy, but it is still vulnerable to prompt injection attacks and stochasticity of GenAI models.
    ...
    _Figure 3. Continuous security with AI agents within digital twins of IT infrastructures._
...
_Figure 4. Meme about decentralized and resilient C2 infrastructure (e.g., [GlassWorm on Solana][glassworm_on_solana], [С2 on social networks/platforms and semi-legitimate traffic][c2_on_semi_legitimate_traffic])._
